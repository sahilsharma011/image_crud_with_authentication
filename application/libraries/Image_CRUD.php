...
class Image_CRUD {
	...
	protected $name_field = 'url';
	protected $url_field = null;
	...
	function set_name_field($name_field)
	{
    ...
    protected function _inset_url($primary_key, $value)
    {
    ...
    protected function _delete_file($id)
    {
    	...
    	unlink( $this->image_path.'/'.$result->{$this->name_field} );
    	unlink( $this->image_path.'/'.$this->thumbnail_prefix.$result->{$this->name_field} );
	...
	function render()
	{
		...
		if(!empty($state_info))
		{
			switch ($state_info->name) {
				...
				case 'insert_title':
					@ob_end_clean();
					$this->_inset_url($_POST['primary_key'],$_POST['value']);
					die();
				break;
			}
		}
